  ...
  interceptor = axiosInstance.interceptors.request.use(
    async (config) => {
      try {
        ...
        const isRefreshTokenExpired = await isRefreshTokenAboutToExpire(
          refreshToken
        );
...
const isRefreshTokenAboutToExpire = async (
  refreshtoken: string,
  extraTimeInSeconds = 30
) => {
  try {
    if (!refreshtoken) {
      return true;
...
export const activateOnboarding = createAction(
  "auth/activateOnboarding",
  (payload: string) => {
